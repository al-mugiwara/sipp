...
class Imunisasi extends CI_Controller {
	...
	public function index()
	{
		...
		$this->template->load('data/template','kader/kegiatan/formImun', $send);
	}
	...
	public function simpan()
	{
		$send = [
			...
			'id_kader'			=> '1',
			'jenis_imunisasi'		=> $this->input->post('imun'),
			'tgl_diberikan'		=> $this->input->post('tglPemberian'),
		];

		$this->m->simpan('tb_imunisasi',$send);
		$session['berhasil'] = "Data Pemberian Imunisasi Berhasil Disimpan";
		$this->session->set_userdata($session);
		redirect('kader/Imunisasi');
	}
